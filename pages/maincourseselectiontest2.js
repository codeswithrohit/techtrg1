...
const handleCheckboxChange = (student) => {
  setSelectedStudents(prevState => {
    ...
    // If the student is already selected, unselect it (remove from selectedStudents)
    if (updatedSelectedStudents[student.armyno]) {
      delete updatedSelectedStudents[student.armyno];
    } else {
      // If the student is not selected, select it
      updatedSelectedStudents[student.armyno] = student;
    }
...
<div className="mt-4">
  ...
  {filteredStudents.map((course, courseIndex) => (
    course.students.map((student, studentIndex) => {
      ...
      // Determine if the student is checked based on maincoursestudents or selectedStudents
