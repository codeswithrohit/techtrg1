...
const SECCDR = ({ courses, isLoading }) => {
  ...
  const bmtCourses = courses.filter(course => course.name === "BMT");
  const amtCourses = courses.filter(course => course.name === "AMT");
      ...
      <div className="max-w-screen-lg mx-auto">
        <div className="flex w-full gap-4">
          <div className="flex-1">
            <h1 className="text-center font-bold text-green-600 text-4xl">BMT</h1>
            {!isLoading && bmtCourses.map((course) => (
              <Accordion key={course._id} title={course.subname} items={course.topics} />
            ))}
          </div>
          <div className="flex-1">
            <h1 className="text-center font-bold text-green-600 text-4xl">AMT</h1>
            {!isLoading && amtCourses.map((course) => (
              <Accordion key={course._id} title={course.subname} items={course.topics} />
            ))}
          </div>
        </div>
