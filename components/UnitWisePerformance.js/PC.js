...
const PC = ({ students,selectedCourses,selectedMaintitles,selectedYears }) => {
    ...
    useEffect(() => {
        const secCdrStudents = students.filter(student => student.course === "PC");
    ...
    const groupedData = uniqueUnitnos.map(unitno => {
        const unitData = {
            unitno,
            performance: {},
            totals: { AI: 0,A: 0,BI: 0, B: 0,C: 0,E: 0 },
        ...
        uniqueYears.forEach(year => {
            ...
            const yearPerformance = {
                ...
                BI: yearData.reduce(
                    (acc, data) =>
                        acc +
                        data.students.filter(
                            student => student.unitno === unitno && student.grading === 'BI'
                        ).length,
                    0
                ),
                B: yearData.reduce(
            ...
            unitData.totals.BI += yearPerformance.BI;
            ...
            cumulativeGoodGradingCount += yearPerformance.AI + yearPerformance.A + yearPerformance.BI;
...
//   console.log('Grouped Data:', groupedData);
  ...
  <h1 className='font-bold text-white bg-red-800 p-4 font-mono underline text-center text-4xl' >PC COURSE UNIT & YR WISE COURSE GRADING</h1>
            ...
            <tr>
              ...
              {uniqueYears.map((year, index) => (
                <React.Fragment key={index}>
                  ...
                  <th className="border border-gray-300 px-4 py-2">BI</th>
                </React.Fragment>
              ))}
              <th className="border border-gray-300 px-4 py-2">AI</th>
              <th className="border border-gray-300 px-4 py-2">A</th>
          ...
          <tbody>
            ...
            {groupedData.map((unitData, unitnoIndex) => (
              <tr key={unitnoIndex}>
                ...
                {uniqueYears.map((year, yearIndex) => {
                  ...
                  return (
                    <React.Fragment key={yearIndex}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{yearPerformance.BI || '-'}</td>
                    </React.Fragment>
                  );
                })}
                {/* Totals */}
                ...
                <td className="border border-gray-300 px-4 py-2">{unitData.totals.BI}</td>
          ...
          <tr>
              ...
              {uniqueYears.map((year) => (
                <React.Fragment key={year}>
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    {groupedData.reduce((total, unitData) => total + (unitData.performance[year]?.BI || 0), 0)}
                  </td>
                </React.Fragment>
              ))}
              ...
              <td className="border border-gray-300 px-4 py-2">
                {groupedData.reduce((total, unitData) => total + unitData.totals.BI, 0)}
              </td>
              <td className="border border-gray-300 px-4 py-2 text-center">
