...
const ADC = ({ students }) => {
    ...
    useEffect(() => {
        ...
        const secCdrStudents = students.filter(student => student.course === "ADC");
    ...
    const selectedCourses = []; // Initialize with default or pass as props
    const selectedMaintitles = [];
    const selectedYears = [];
    ...
    const groupedData = uniqueUnitnos.map(unitno => {
        ...
        uniqueYears.forEach(year => {
            ...
            cumulativePoorGradingCount += yearPerformance.AY + yearPerformance.BX;
        });
...
//   console.log('Grouped Data:', groupedData);
  ...
  <h1 className='font-bold text-white bg-red-800 p-4 font-mono underline text-center text-4xl' >ADC COURSE UNIT & YR WISE COURSE GRADING</h1>
