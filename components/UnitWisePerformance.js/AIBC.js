...
const AIBC = ({ students,selectedCourses,selectedMaintitles,selectedYears }) => {
    const [filteredStudentsdata, setFilteredStudentsdata] = useState([]);
    useEffect(() => {
        const secCdrStudents = students.filter(student => student.course === "AIBC");
    ...
    const groupedData = uniqueUnitnos.map(unitno => {
        const unitData = {
            unitno,
            performance: {},
            totals: { AX_I: 0,AXR: 0, AX: 0, AY: 0, BXI: 0, BXR: 0,BX: 0,BY: 0,CX: 0,CZ: 0 },
        ...
        uniqueYears.forEach(year => {
            ...
            const yearPerformance = {
                ...
                AXR: yearData.reduce(
                    (acc, data) =>
                        acc +
                        data.students.filter(
                            student => student.unitno === unitno && student.grading === 'AXR'
                        ).length,
                    0
                ),
                AY: yearData.reduce(
                ...
                BXI: yearData.reduce(
                    (acc, data) =>
                        acc +
                        data.students.filter(
                            student => student.unitno === unitno && student.grading === 'BXI'
                        ).length,
                    0
                ),
                BXR: yearData.reduce(
                    (acc, data) =>
                        acc +
                        data.students.filter(
                            student => student.unitno === unitno && student.grading === 'BXR'
                        ).length,
                    0
                ),
                BX: yearData.reduce(
                ...
                CZ: yearData.reduce(
                    (acc, data) =>
                        acc +
                        data.students.filter(
                            student => student.unitno === unitno && student.grading === 'CZ'
                        ).length,
                    0
                ),
            };
            ...
            unitData.totals.AXR += yearPerformance.AXR;
            unitData.totals.AY += yearPerformance.AY;
            unitData.totals.BXI += yearPerformance.BXI;
            unitData.totals.BXR += yearPerformance.BXR;
            unitData.totals.BX += yearPerformance.BX;
            unitData.totals.BY += yearPerformance.BY;
            unitData.totals.CX += yearPerformance.CX;
            unitData.totals.CZ += yearPerformance.CZ;
            ...
            cumulativeGoodGradingCount += yearPerformance.AX_I + yearPerformance.AXR + yearPerformance.AX;
            cumulativePoorGradingCount += yearPerformance.AY + yearPerformance.BXI + yearPerformance.BXR + yearPerformance.BX + yearPerformance.BY + yearPerformance.CX + yearPerformance.CZ ;
        });
...
//   console.log('Grouped Data:', groupedData);
  ...
  <h1 className='font-bold text-white bg-red-800 p-4 font-mono underline text-center text-4xl' >AIBC COURSE UNIT & YR WISE COURSE GRADING</h1>
          ...
          <thead className="bg-gray-200">
            <tr>
              ...
              {uniqueYears.map((year, index) => (
                <th key={index} className="border border-gray-300 px-4 py-2" colSpan="4">
                  {year}
                </th>
              ))}
              <th className="border border-gray-300 px-4 py-2" colSpan="3">TOTAL</th>
            ...
            <tr>
              ...
              {uniqueYears.map((year, index) => (
                <React.Fragment key={index}>
                  ...
                  <th className="border border-gray-300 px-4 py-2">AXR</th>
          ...
          <tbody>
            ...
            {groupedData.map((unitData, unitnoIndex) => (
              <tr key={unitnoIndex}>
                ...
                {uniqueYears.map((year, yearIndex) => {
                  ...
                  return (
                    <React.Fragment key={yearIndex}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{yearPerformance.AXR || '-'}</td>
                ...
                <td className="border border-gray-300 px-4 py-2">{unitData.totals.AXR}</td>
  ...
  {totalGoodGrading.STR > 0 ? ((unitData.goodGrading.STR * 100) / totalGoodGrading.STR).toFixed(2) : '0'}%
</td>
          ...
          <tr>
              ...
              {uniqueYears.map((year) => (
                <React.Fragment key={year}>
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    {groupedData.reduce((total, unitData) => total + (unitData.performance[year]?.AXR || 0), 0)}
                  </td>
                  <td className="border border-gray-300 px-4 py-2">
              ...
              <td className="border border-gray-300 px-4 py-2">
                {groupedData.reduce((total, unitData) => total + unitData.totals.AXR, 0)}
              </td>
              <td className="border border-gray-300 px-4 py-2">
