...
const PC = ({filteredStudents,handleDeleteStudent,maincoursestudents,filteredInstructors}) => {
...
const handlePrint = () => {
  ...
  return (
    ...
    const pre1stBI = student.students.filter(s => s.phase === 'pre1st' && s.grading === 'BI').length;
    const pre2ndBI = student.students.filter(s => s.phase === 'pre2nd' && s.grading === 'BI').length;

    const totalAI = pre1stAI + pre2ndAI;
    const totalA = pre1stA + pre2ndA;
    const totalBI = pre1stBI + pre2ndBI;
    ...
    const percentageBI = allStudentLength > 0 ? (totalBI / (totalstudents)) * 100 : 0;

    const combinedTotal = totalAI + totalA + totalBI ;
            const combinedPercentage = totalstudents > 0 ? ((totalAI + totalA + totalBI ) / totalstudents) * 100 : 0;
    ...
    return (
      <tr className='cursor-pointer'  key={student._id}>
        ...
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{pre1stBI}</td>
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{pre2ndBI}</td>
        ...
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{percentageBI.toFixed(2)}%</td>
...
{student.bi ? student.bi : '-'}
        ...
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">      {student.totalArmyStrength > 0 ? ((student.bi) / student.totalArmyStrength * 100).toFixed(2) + '%' : '0%'}</td>
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300"> {student.totalArmyStrength > 0 ? ((student.aI+ student.bi + student.totalStrengthA) / student.totalArmyStrength * 100).toFixed(2) + '%' : '0%'}</td>
  ...
  <div className="bg-white p-6 rounded-lg shadow-lg w-full h-full overflow-y-auto">
            ...
            <h2 className="font-bold text-white bg-red-800 p-4 font-mono underline text-center text-4xl mb-2">RESULT SHEET OF {modalData.Course} COURSE {modalData.mainCourse}  AT BELGAUM {modalData.targetYear} </h2>
  ...
  <div className="mt-4 p-4 bg-gray-200 rounded-lg">
        ...
        <table className="w-full border border-gray-300">
          ...
          <tbody className="divide-y divide-gray-800">
            ...
            <tr>
              <td className="px-4 py-2 text-center border-r border-gray-800">BI</td>
              <td className="px-4 py-2 text-center">{gradingSummary.BI}</td>
            </tr>
            <tr>
