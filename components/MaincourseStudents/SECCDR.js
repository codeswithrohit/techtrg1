...
const SECCDR = ({filteredStudents,handleDeleteStudent,maincoursestudents,filteredInstructors}) => {
  ...
  const handleCourseClick = (maincourse,mainid,precourse,course) => {
  ...
  setInstructordata(filteredInstructorsByStudentDate)
    const studentsInCourse = maincoursestudents.find(student => student.maincourse === maincourse && student.precourse === precourse)?.students || [];
    console.log("studnetincourse",studentsInCourse)
    console.log("maincoursestudents",maincoursestudents)
    setModalStudents(studentsInCourse);
    setModalData({
      targetYear: filteredStudents.find(student => student.maincourse === maincourse)?.targetYear,
      Course: filteredStudents.find(student => student.maincourse === maincourse && student._id === mainid)?.course,
...
const [gradingSummary, setGradingSummary] = useState({ AXI: 0, AX: 0, AY: 0, BX: 0, BY: 0,CX: 0, CY: 0, CZ:0,E:0 });

// Function to calculate the grading summary
const calculateGradingSummary = () => {
  const summary = { AXI: 0, AX: 0, AY: 0, BX: 0, BY: 0,CX: 0, CY: 0, CZ:0,E:0 };
...
const handlePrint = () => {
  ...
  return (
    ...
    const combinedTotal = totalAXI + totalAX;
            const combinedPercentage = totalstudents > 0 ? ((totalAXI + totalAX) / totalstudents) * 100 : 0;



    return (
      <tr className='cursor-pointer'  key={student._id}>
        <td onClick={() => handleCourseClick(student.maincourse,student._id,student.precourse,student.course)} className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{index + 1}</td>
        <td onClick={() => handleCourseClick(student.maincourse,student._id,student.precourse,student.course)} className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{student.maincourse}</td>
        <td onClick={() => handleCourseClick(student.maincourse,student._id,student.precourse,student.course)} className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">{allStudentLength}</td>
...
<td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300">
  {student.totalStrengthAX ? student.totalStrengthAX : '-'}
        ...
        <td className="px-4 py-1 text-center text-sm text-gray-500 border-r border-gray-300"> {student.totalArmyStrength > 0 ? ((student.axI+student.totalStrengthAX) / student.totalArmyStrength * 100).toFixed(2) + '%' : '0%'}</td>
        ...
        <table className="w-full border border-gray-300">
          ...
          <tbody className="divide-y divide-gray-800">
            ...
            <tr>
              ...
              <td className="px-4 py-2 text-center">{gradingSummary.CX}</td>
            </tr>
            <tr>
              <td className="px-4 py-2 text-center border-r border-gray-800">CY</td>
              <td className="px-4 py-2 text-center">{gradingSummary.CY}</td>
            </tr>
