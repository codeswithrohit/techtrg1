...
const PC = ({ maincoursestudents, selectedCourse, selectedYears,selectedMainCourse }) => {
    ...
    const handleDoughnutClick = (event, elements) => {
        ...
        let gradingType = ["AI", "A","BI", "Other Graded"][index];
        ...
        const filteredStudents = maincoursestudents
            ...
            .filter(student => {
                if (gradingType === "AI") return student.grading === "AI";
                if (gradingType === "A") return student.grading === "A";
                if (gradingType === "BI") return student.grading === "BI";
                return student.grading !== "AI" && student.grading !== "A" && student.grading !== "BI";
            });
    ...
    const mergedData = Data.reduce((acc, courseData) => {
        ...
        courseData.students.forEach(student => {
            if (student.grading === "AI") {
                acc.axIStudentsCount += 1;
            } else if (student.grading === "A") {
                acc.axStudentsCount += 1;
            } else if (student.grading === "BI") {
                acc.biStudentsCount += 1;
            } else {
        ...
        return acc;
    }, { vacAllotted: 0, vacUtilised: 0, axIStudentsCount: 0, axStudentsCount: 0,biStudentsCount: 0, otherStudentsCount: 0 });
    ...
    const totalStudents = mergedData.axIStudentsCount + mergedData.axStudentsCount + mergedData.biStudentsCount + mergedData.otherStudentsCount;

    const getPercentage = (count) => (totalStudents > 0 ? ((count / totalStudents) * 100).toFixed(1) + '%' : '0%');
    
    const doughnutChartData = {
        labels: [`AI (${getPercentage(mergedData.axIStudentsCount)})`, `A (${getPercentage(mergedData.axStudentsCount)})`, `BI (${getPercentage(mergedData.biStudentsCount)})`, `Other Graded (${getPercentage(mergedData.otherStudentsCount)})`],
        datasets: [
            {
                label: 'Grading Distribution',
                data: [mergedData.axIStudentsCount, mergedData.axStudentsCount,mergedData.biStudentsCount, mergedData.otherStudentsCount],
                backgroundColor: ['rgba(15, 10, 222, 0.7)', 'rgba(60, 179, 113, 0.7)', 'rgb(249, 10, 168)', 'rgba(255, 165, 0, 0.7)'],
                borderWidth: 1,
            }
        ]
    };
