...
const AIBC = ({ maincoursestudents, selectedCourse, selectedYears,selectedMainCourse }) => {
    ...
    const handleDoughnutClick = (event, elements) => {
        ...
        let gradingType = ["AX(I)", "AX","AXR", "Other Graded"][index];
        ...
        const filteredStudents = maincoursestudents
            ...
            .filter(student => {
                ...
                if (gradingType === "AXR") return student.grading === "AXR";
                return student.grading !== "AX(I)" && student.grading !== "AX" && student.grading !== "AXR";
            });
    ...
    const mergedData = Data.reduce((acc, courseData) => {
        ...
        courseData.students.forEach(student => {
            ...
            }  else if (student.grading === "AXR") {
                acc.axrStudentsCount += 1;
            } else {
        ...
        return acc;
    }, { vacAllotted: 0, vacUtilised: 0, axIStudentsCount: 0, axStudentsCount: 0,axrStudentsCount: 0, otherStudentsCount: 0 });
    ...
    const totalStudents = mergedData.axIStudentsCount + mergedData.axStudentsCount + mergedData.axrStudentsCount + mergedData.otherStudentsCount;

    const getPercentage = (count) => (totalStudents > 0 ? ((count / totalStudents) * 100).toFixed(1) + '%' : '0%');
    
    const doughnutChartData = {
        labels: [`AX(I) (${getPercentage(mergedData.axIStudentsCount)})`, `AX (${getPercentage(mergedData.axStudentsCount)})`, `AXR(${getPercentage(mergedData.axrStudentsCount)})`, `Other Graded (${getPercentage(mergedData.otherStudentsCount)})`],
        datasets: [
            {
                label: 'Grading Distribution',
                data: [mergedData.axIStudentsCount, mergedData.axStudentsCount, mergedData.axrStudentsCount, mergedData.otherStudentsCount],
