...
const MTCADRE = ({ maincoursestudents, selectedCourse, selectedYears,selectedMainCourse }) => {
    ...
    const handleDoughnutClick = (event, elements) => {
        ...
        let gradingType = ["PASS", "Other Graded"][index];
        ...
        const filteredStudents = maincoursestudents
            ...
            .filter(student => {
                if (gradingType === "PASS") return student.grading === "PASS";
                return student.grading !== "PASS";
            });
    ...
    const mergedData = Data.reduce((acc, courseData) => {
        ...
        return acc;
    }, { vacAllotted: 0, vacUtilised: 0, axIStudentsCount: 0,  otherStudentsCount: 0 });
    ...
    const totalStudents = mergedData.axIStudentsCount +  mergedData.otherStudentsCount;

    const getPercentage = (count) => (totalStudents > 0 ? ((count / totalStudents) * 100).toFixed(1) + '%' : '0%');
    
    const doughnutChartData = {
        labels: [`PASS (${getPercentage(mergedData.axIStudentsCount)})`, `Other Graded (${getPercentage(mergedData.otherStudentsCount)})`],
        datasets: [
            {
                label: 'Grading Distribution',
                data: [mergedData.axIStudentsCount, mergedData.otherStudentsCount],
